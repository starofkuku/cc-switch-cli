...
- **CLI / Codex sessions export**: Export the complete rollout into a lossless structured JSON archive under `.tmp` by default, with event statistics and indexes for decisions, files, Git state, commands, validations, patches, commits, handoff summaries, and pending work. Also generate a redacted project-root context document; non-Codex exports remain `ccswitch-session` v1.
...
- **CLI / Apps list**: `cc-switch apps list` prints supported app labels as a comma-separated line matching `--app` values (`claude, codex, gemini, open-code, hermes, open-claw, pi, grok`). No database access required.

## [5.10.5] - 2026-07-21

### Changed

- **CLI / Sessions export (interactive)**: Default flow is now two-step — pick a **work directory** first (basename only; sessions without `projectDir` are hidden), then pick a session under it. Pre-selects the group matching the process cwd when possible. `--id` still exports directly without workdir selection.
